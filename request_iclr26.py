...
PAPER_VENUE_ID = "ICLR.cc/2026/Conference/Submission"

# API 请求参数配置
API_PARAMS = {
    "content.venueid": PAPER_VENUE_ID,
...
class ICLR26Crawler:
    ...
    def __init__(self, limit: int = 25, output_format: str = "json", delay: float = 0.8):
        ...
        # 根据 PAPER_VENUE_ID 动态生成输出文件名
        self.output_file = f"iclr26_all_papers.{self.output_format}"
    ...
    def fetch_all_papers(self) -> List[Dict]:
        ...
        print("=" * 60)
        print(f"论文类型: {API_PARAMS.get('content.venueid')}")
    ...
    def save_data(self, papers: List[Dict]) -> None:
        ...
        if self.output_format == "json":
            ...
            self.output_file = "iclr26_papers.json"
            self.save_as_json(papers)
...
def main():
    ...
    try:
        # 创建爬虫实例
        crawler = ICLR26Crawler(limit=LIMIT, output_format=OUTPUT_FORMAT, delay=INITIAL_DELAY)
